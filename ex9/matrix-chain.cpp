...
pair<vector<vector<ll>>, vector<vector<ll>>> matrix_chain_order(const vector<ll> &r) {
  int n = r.size() - 1;
  auto m = make_vector(n + 1, n + 1, 0LL);
  auto s = make_vector(n + 1, n + 1, 0LL);
  ...
  for (int l = 2; l <= n; l++) {
    for (int i = 1; i <= n - l + 1; i++) {
      ...
      for (int k = i; k <= j - 1; k++) {
        ll q = m[i][k] + m[k + 1][j] + r[i - 1] * r[k] * r[j];
        if (q < m[i][j]) {
          m[i][j] = q;
          s[i][j] = k;
        }
  ...
  return pair(vector<vector<ll>>(m), vector<vector<ll>>(s));
}


void print_optimal_parens(const vector<vector<ll>> &s, int i, int j) {
  if (i == j) {
    cout << char('A' + i);
    return;
  }
...
int main() {
  ...
  cout << "input list of dimensions >>>" << flush;
  vector<ll> r(n);
  cin >> r;

  auto [m, s] = matrix_chain_order(r);

  print_optimal_parens(s, 0, len(m) - 1);
  cout << endl;

  cout << "m:" << endl;
  for (int i = 0; i < len(m); i++) {
    for (int j = 0; j < len(m[i]); j++) {
      cout << setw(8) << m[i][j];
    }
    cout << endl;
  }
